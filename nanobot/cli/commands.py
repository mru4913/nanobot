...
def _run_gateway(
    ...
    *,
    ...
    async def _gleezy_compat_server(host: str, compat_port: int) -> None:
        """Gleezy REST API + /health on the gateway port (18790)."""
        console.print(
            f"[green]✓[/green] Gleezy compat API: http://{host}:{compat_port}/health"
        )
        await start_compat_server(
            host,
            compat_port,
            bus=bus,
            session_manager=session_manager,
    ...
    async def run():
        try:
            ...
            tasks = [
                agent.run(),
                channels.start_all(),
                _gleezy_compat_server(config.gateway.host, port),
            ]
            if open_browser_url:
